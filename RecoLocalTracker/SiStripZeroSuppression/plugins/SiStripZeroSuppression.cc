	...
	
{
  ...
  for(tag_iterator_t inputTag = inputTags.begin(); inputTag != inputTags.end(); ++inputTag ){
    ...
    produces< edm::DetSetVector<SiStripDigi> > ("ZeroSuppressedNBandT"+inputTag->instance());
...
inline void SiStripZeroSuppression::StandardZeroSuppression(edm::Event& e){
  ...
  for(tag_iterator_t inputTag = inputTags.begin(); inputTag != inputTags.end(); ++inputTag,++inputToken ) {
    ...
    if (input->size())
      ...
      std::auto_ptr< edm::DetSetVector<SiStripDigi> > outputNBandT(new edm::DetSetVector<SiStripDigi>(output_baseNBandT) );
      e.put( outputNBandT, "ZeroSuppressedNBandT"+inputTag->instance() );
...
processRaw(const edm::InputTag& inputTag, const edm::DetSetVector<SiStripRawDigi>& input ) {
  ...
  output_baseNBandT.reserve(16000);
  ...
  for ( edm::DetSetVector<SiStripRawDigi>::const_iterator 
      ...
      edm::DetSet<SiStripDigi> suppressedDigisNBandT(rawDigis->id);
      ...
      if ( "ProcessedRaw" == inputTag.instance()) nAPVflagged = algorithms->SuppressProcessedRawData(*rawDigis, suppressedDigis);
               ...
               {
                    //std::cout << "supressing VR" << std::endl; 
                    nAPVflagged = algorithms->SuppressVirginRawData(*rawDigis, suppressedDigis, suppressedDigisNBandT); 
                    }
      else     
	...
	output_base.push_back(suppressedDigis); 
      ...
      if (suppressedDigisNBandT.size() && (storeInZScollBadAPV || nAPVflagged ==0))
      { 
	output_baseNBandT.push_back(suppressedDigisNBandT); 
      }
