...
class SiStripBaselineAnalyzerSlim2 : public edm::EDAnalyzer {
   ...
   private:
	  ...
	  uint16_t actualModule_;
          ...
          std::vector< std::vector<uint16_t> > maxADCOfClustersMtx;
          std::vector< std::vector<float> > NCchargeOfClustersMtx;
          std::vector< std::vector<uint16_t> > NCcenterOfClustersMtx;
          std::vector< std::vector<uint16_t> > NCstripsTouchedOfClustersMtx;
          std::vector< std::vector<uint16_t> > NCmaxADCOfClustersMtx;
...
*/
  ...
  timeTree->Branch("maxADCOfClustersMtx", "std::vector < std::vector<uint16_t> >", &maxADCOfClustersMtx);
  timeTree->Branch("NCchargeOfClustersMtx", "NCstd::vector < std::vector<float> >", &NCchargeOfClustersMtx);
  timeTree->Branch("NCcenterOfClustersMtx", "std::vector < std::vector<uint16_t> >", &NCcenterOfClustersMtx);
  timeTree->Branch("NCstripsTouchedOfClustersMtx", "std::vector < std::vector<uint16_t> >", &NCstripsTouchedOfClustersMtx);
  timeTree->Branch("NCmaxADCOfClustersMtx", "std::vector < std::vector<uint16_t> >", &NCmaxADCOfClustersMtx);
...
SiStripBaselineAnalyzerSlim2::analyze(const edm::Event& e, const edm::EventSetup& es)
{
   ...
   NCchargeOfClustersMtx.clear();
   NCcenterOfClustersMtx.clear();
   NCstripsTouchedOfClustersMtx.clear();
   NCmaxADCOfClustersMtx.clear();
 ...
 //  string measurementTrackerLabel_ = "";
   ...
   es.get<CkfComponentsRecord>().get(measurementTracker);
   edm::Handle<MeasurementTrackerEvent> measurementTrackerEvent;
   e.getByToken(trackerEvent_token_,measurementTrackerEvent);

   ///hit efficiency
   //
   const LayerMeasurements* theLayerMeasurements = NULL;
   theLayerMeasurements = new LayerMeasurements(*measurementTracker, *measurementTrackerEvent);
   cout << "theLayerMeasurements  " << theLayerMeasurements << endl; 
*/
   ...
   //cout <<  "moduleRawDigi.size(): " << moduleRawDigi.size() << endl;
...
//                else std::cout << "Resynched..." << std::endl;
      ...
      //cout << "ProcessedRawDigis.size(): " <<  ProcessedRawDigis.size() << endl;
	...
	++strip;
      ...
      for(uint32_t ccol = 0; ccol<2; ccol++  )
      {
          int nclust = 0;
          ...
          std::vector<uint16_t> clustersMaxADC;
          ...
          {
              itClusters = NCclusters->begin();
              itClustersEnd = NCclusters->end();
