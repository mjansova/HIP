...
int main(int argc, char *argv[])
{
    ...
    TH1F* blBefore = new TH1F("blBefore", "blBefore", lTrain,-lTrain,0);
     ...
     TH2F* BXvsRMS = new TH2F("BXvsRMS", "BXvsRMS", 2*lTrain, -lTrain, lTrain, 100  , 0, 50 );
     ...
     TH2F* BXvsWI = new TH2F("BXvsWI", "BXvsWI", 2*lTrain, -lTrain, lTrain, 20, 0, 20);
    ...
    for (Int_t m=0; m<nentries; m++) //ATTENTION looping over modules not events
    {
		...
		//histos.clusHist = new TH1D( hName + "clus", hName + "clus", 768768, 0, 768768);
            ...
            for(uint32_t b = 0; b<baselines.size(); b++)
            {
                ...
                //cout << "here3 " << endl;
                if(baselinesRMS.at(b) < 2.5 && baselines.at(b) < -25 )
    ...
    */
    ...
    for(uint32_t b = 0; b<moduleHIP.at(0).GetNbinsX(); b++ )
    {
        for(uint32_t hi = 0 ; hi<moduleHIP.size(); hi++)
        {
            ...
            if( moduleHIP.at(hi).GetBinContent(b+1) != 0)
            {
                ...
                binEntry++;
                baseline2D->Fill(moduleHIP.at(hi).GetBinLowEdge(b+1), moduleHIP.at(hi).GetBinContent(b+1));
                baselineP->Fill(moduleHIP.at(hi).GetBinLowEdge(b+1), moduleHIP.at(hi).GetBinContent(b+1));
            }
    ...
    TCanvas* cHIPaver = new TCanvas("CHIPaver", "cHIPaver");
    cHIPaver->cd();
    HIPaver->SetTitle("");
    ...
    TCanvas* cHIPm = new TCanvas("CHIPm", "cHIPm");
    cHIPm->cd();
    HIPmult->SetTitle("");
