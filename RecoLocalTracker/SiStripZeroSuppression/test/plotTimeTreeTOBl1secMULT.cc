...
int main(int argc, char *argv[])
{
    ...
    //TH1F* blBefore = new TH1F("blBefore", "blBefore", lTrain,-lTrain,0);
     ...
     TH2F* cleanedBXvsCH = new TH2F("cleanedBXvsCH", "cleanedBXvsCH", 2*lTrain, -lTrain, lTrain, 10000  , 0, 20000 );
     TProfile* cleanedPBXvsCH = new TProfile("cleanedPBXvsCH", "cleanedPBXvsCH", 2*lTrain, -lTrain, lTrain );
     cleanedBXvsCH->GetYaxis()->SetTitle("charge[ADC]");
     cleanedBXvsCH->GetXaxis()->SetTitle("bunch crossing");
     cleanedPBXvsCH->GetYaxis()->SetTitle("charge[ADC]");
     cleanedPBXvsCH->GetXaxis()->SetTitle("bunch crossing");
     ...
     TProfile* PBXvsCHall = new TProfile("PBXvsCHall", "PBXvsCHall", 3000, 0, 3000 );
     ...
     TProfile* PBXvsCHall2 = new TProfile("PBXvsCHall2", "PBXvsCHall2", 3000, 0, 3000 );
     ...
     TProfile* PBXvsWI = new TProfile("PBXvsWI", "PBXvsWI", 2*lTrain, -lTrain, lTrain);
     BXvsWI->SetTitle("");
     ...
     TH2F* BXvsWIall = new TH2F("BXvsWIall", "BXvsWIall", 2*lTrain, -lTrain, lTrain, 100, 0, 100);
     TProfile* PBXvsWIall = new TProfile("PBXvsWIall", "PBXvsWIall", 2*lTrain, -lTrain, lTrain);
     BXvsWIall->SetTitle("");
    ...
    //std::map<uint32_t, vector< TGraph*> > moduleMap;
     ...
     TH1F* decrease = new TH1F("blDecreaseid", "blDecrease", 2*lTrain, -lTrain, lTrain);
     TH1F* increase = new TH1F("blIncreased", "blIncrease", 2*lTrain, -lTrain, lTrain);
     TH1F* reference = new TH1F("reference", "reference",  2*lTrain, -lTrain, lTrain);
     TH1F* fDecreased = new TH1F("fDecreasedBl", "fDecreasedBl",  2*lTrain, -lTrain, lTrain);
     TH1F* fIncreased = new TH1F("fIncreasedBl", "fIncreasedBl",  2*lTrain, -lTrain, lTrain);

     TProfile* HIPcluster = new TProfile("HIPclusterCH", "HIPclusterCH", 2*lTrain, -lTrain, lTrain, "S" );
     TH1F* fHIPClus = new TH1F("fOfHIPClusters", "fOfHIPClusters", 2*lTrain,-lTrain, lTrain);
     ...
     TH1F* whichBXIsHIP =  new TH1F("whichBXIsHIP", "whichBXIsHIP", 2, 0, 2);
     TProfile* multProfAll = new TProfile(" multProfAll", " multProfAll", 3000, 0, 3000);
     TProfile* multProfAllHIP = new TProfile(" multProfAllHIP", " multProfAllHIP", 2*lTrain,-lTrain, lTrain);
     TProfile* cleanedmultProfAllHIP = new TProfile(" cleanedmultProfAllHIP", " cleanedmultProfAllHIP", 2*lTrain,-lTrain, lTrain);
...
//
		...
		//histos.clusHist = new TH1D( hName + "clus", hName + "clus", 768768, 0, 768768);
            ...
            for(uint32_t b = 0; b<baselines.size(); b++)
            {
                ...
                multProfAll->Fill(bunchNrsOUT->at(i),CMultiplicity);
                if(CMultiplicity != 0)
                ...
                //cout << "here4 " << endl;
                    ...
                    evfile << moduleKeyOUT << " " << eventNrsOUT->at(i) << " " << "HIP" << " " << HIPnr <<endl;  //I need to save module ID!!!!
                    if(i>0)
                    {
                        float baseline = baselines.at(b) - baselinesMtxOUT->at(i-1).at(b);
                        reference->Fill((Double_t)0);
                        if(baseline<0)
                            decrease->Fill((Double_t)0);
                        else
                            increase->Fill((Double_t)0);
                    }
                ...
                //cout << "here5 " << endl;
                    ...
                    multProfAllHIP->Fill((Double_t)0, mult);
                    if(bunchNrsOUT->at(i) > 334)
                        cleanedmultProfAllHIP->Fill((Double_t)0, mult);
                    ...
                    if(HIPcen>0)
                    {
                        ...
                        HIPcluster->Fill((Double_t)0,HIPch);
                        fHIPClus->Fill((Double_t)0);
                        nrOfHIPClus++;
                    }
                    if(maxchrg>0)
                    {
                          ...
                          if(bunchNrsOUT->at(i) > 334)
                          {
                              cleanedBXvsCH->Fill((Double_t)0 , maxchrg);
                              cleanedPBXvsCH->Fill((Double_t)0 , maxchrg);
                          }
         ...
         uint32_t maxstrpBef = 0;
                ...
                //cout << "here7 " << endl;
                            if(*it2 > 255 && *it2 < 384)
                            {
                                ...
                                cenBef = *it2;
                                multBef++;
                                if(filled == false && HIPcen>0 && (cenBef <= HIPcen+5 && cenBef >= HIPcen-5 ))
                                {
                                    cout << "cen Bef: " << cenBef << endl;
                                    HIPcluster->Fill((Double_t)bunchNrsOUT->at(i-1)-bunchNrsOUT->at(i),chrgBef);
                                    fHIPClus->Fill((Double_t)bunchNrsOUT->at(i-1)-bunchNrsOUT->at(i));
                                    filled = true;
                                }
                        ...
                        multProfAllHIP->Fill((Double_t)bunchNrsOUT->at(i-1)-bunchNrsOUT->at(i), multBef);
                        if(bunchNrsOUT->at(i-1) > 334)
                            cleanedmultProfAllHIP->Fill((Double_t)bunchNrsOUT->at(i-1)-bunchNrsOUT->at(i), multBef);
                        if(maxchrgBef>0)
                        { 
                               ...
                               if(bunchNrsOUT->at(i-1) > 334)
                               {
                                     cleanedBXvsCH->Fill((Double_t)bunchNrsOUT->at(i-1)-bunchNrsOUT->at(i) ,maxchrgBef);
                                     cleanedPBXvsCH->Fill((Double_t)bunchNrsOUT->at(i-1)-bunchNrsOUT->at(i) ,maxchrgBef);
                                }
                        ...
                        evfile << moduleKeyOUT << " " << eventNrsOUT->at(i) << " " << "PRE" << " " << HIPnr << endl;  //I need to save module ID!!!!
                        //
                //cout << "here8 " << endl;
                ...
                //cout << "here9 " << endl;
                    ...
                    if(i>0 && baselines.size()>3 && baselinesMtxOUT->at(i-1).size()>3)
                    {
                    float baseline = baselines.at(b) - baselinesMtxOUT->at(i-1).at(b);
                    reference->Fill(deltaBunch);
                    if(baseline<0)
                        decrease->Fill(deltaBunch);
                    else
                        increase->Fill(deltaBunch);
                    }
                ...
                //cout << "here9.3 " << endl;
                    ...
                    for(std::vector<uint16_t>::iterator it = clusterCen.begin(); it != clusterCen.end(); ++it)
                    {
                      if(*it > 255 && *it < 384)
                      {
                          ...
                          cenAf =  *it;
                          if(filled2 == false &&HIPcen>0 && (cenAf <= HIPcen+5 && cenAf >= HIPcen-5 ))
                          {
                              cout << "cenAf " << cenAf << endl;
                              HIPcluster->Fill(deltaBunch, chrgAf);
                              fHIPClus->Fill(deltaBunch);
                              filled2 = true;
                          }
                          BXvsCH2->Fill(deltaBunch , clusterADC.at(counter));
                          PBXvsCH2->Fill(deltaBunch , clusterADC.at(counter));
 
                          largeClusPerMod->Fill(moduleValueOUT);
                    ...
                    multProfAllHIP->Fill(deltaBunch, multAf);
                    if(bunchNrsOUT->at(i) > 334)
                        cleanedmultProfAllHIP->Fill(deltaBunch, multAf);
                //cout << "here9.4 " << endl;
                    if(maxchrgAf>0)
                    {
                          ...
                          if(bunchNrsOUT->at(i) > 334)
                          {
                              cleanedBXvsCH->Fill(deltaBunch , maxchrgAf);
                              cleanedPBXvsCH->Fill(deltaBunch , maxchrgAf);
                          }
